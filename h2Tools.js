(async() => {
	...
	async function writeRecords(recordsObject = null, outputName = null, tableName = "PLEX")
	{
		...
		if(currentGameYear >= 26)
		{
			...
			const visualType = prompt().trim().toLowerCase();
			
			const trailingBuf = visualType === 'c' ? Buffer.from([0xD3, 0x29, 0x66, 0x00, 0x83, 0xBD, 0x9A, 0x84, 0x09, 0x00]) : Buffer.from([0xD3, 0x29, 0x66, 0x00, 0x90, 0xB1, 0x8A, 0x94, 0x0B, 0x00]);
